...
const { User, UserVariable, Settings, OfferTemplate, UserPlan, Lead, Currency, Transaction, Plan } = db;
...
const AdminNewUserTemplate = require('../EmailTemplate/AdminNewUserTemplate');
...
exports.register = async (req, res) => {
  try {
    ...
    const defaultCurrency = await Currency.findOne({ where: { code: 'DKK' } });
    if (defaultCurrency) {
      await user.update({ currencyId: defaultCurrency.id });
    }

    // -------------------- SEND ADMIN NOTIFICATION EMAIL --------------------
    try {
      const adminUser = await User.findByPk(1); // ✅ admin is fixed id = 1
      ...
      if (adminUser && adminUser.email) {
        ...
        await sendMail(
          adminUser.id, // ✅ admin userId (for logs/history)
          {
            to: adminUser.email, // ✅ admin email from DB
            subject: 'New User Registered - NaviLead',
      ...
      // ❗ Registration must NOT fail because of email
