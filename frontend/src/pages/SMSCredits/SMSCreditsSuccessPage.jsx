...
const SubscriptionSuccessPage = () => {
    ...
    const verifySession = async () => {
      try {
        ...
        const response = await api.post(
          '/smsCredits/verify-session',
          { sessionId },
          { headers: { Authorization: `Bearer ${authToken}` } }
        );

        setUserPlanData(response.data);
        setApiMessage(response.data.message || 'Payment Done successfully!');
        toast.success(response.data.message || 'Payment Done successfully!');
      } catch (error) {
        console.error(error);
        const msg =
  ...
  }, [sessionId, authToken, setUserPlan]);
        ...
        {errorMsg ? (
          ...
          <>
            ...
            <div className="text-left bg-gray-50 p-4 rounded-md text-sm mb-4">
              <p>
                <strong>Status:</strong> {userPlanData?.transaction?.status}
              </p>
              <p>
