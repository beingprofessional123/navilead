...
const TicketPage = () => {
    ...
    const handleSoundSync = async (toggleValue = null) => {
        try {
            ...
            const res = await api.post('/admin/tickets/update-setting', payload,
                {
                    headers: {
        ...
        const mainInterval = setInterval(async () => {
            try {
                ...
                if (isChatModalOpen && selectedTicket) {
                    const msgRes = await api.get(`/admin/tickets/${selectedTicket.id}/messages`, {
                        headers: { Authorization: `Bearer ${authToken}` }
                    });
                ...
                localStorage.setItem('AdminprevUnreadCount', currentTotalUnread);
                                    ...
                                    <form onSubmit={handleSendMessage}>
                                        ...
                                        <div className="row mb-2">
                                            ...
                                            <div className="col-md-3 mb-2">
                                                ...
                                                <div>
                                                    <select
                                                        className={`form-select form-select-sm border-0 shadow-sm ${selectedTicket.status === 'Resolved' ? 'bg-success-subtle text-success' :
                                                            selectedTicket.status === 'Pending' ? 'bg-warning-subtle text-warning' :
                                                                selectedTicket.status === 'Closed' ? 'bg-danger-subtle text-danger' :
                                            ...
                                            {chatMessages.length > 0 ? (
                                                <>
                                                    {chatMessages.map((msg, index) => (
                                                        /* Yahan Logic Change: Agar admin hai to Right (end), agar user hai to Left (start) */
                                                        <div key={index} className={`d-flex align-items-start mb-3 ${msg.senderType === 'admin' ? 'justify-content-end' : ''}`}>

                                                            {/* AVATAR LEFT (Ab User ka logo left side aayega) */}
                                                            {msg.senderType === 'user' && (
                                                                <div className="avatar me-2" style={{ minWidth: '35px', height: '35px', borderRadius: '50%', overflow: 'hidden', background: '#6c757d', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                                    {msg.receiver?.companyLogo ? (
                                                                        <img
                                                                            src={msg.receiver.companyLogo}
                                                                            alt="ME"
                                                            ...
                                                            <div className={`msg`} style={{ maxWidth: '75%' }}>
                                                                ...
                                                                {msg.attachment && (
                                                                    /* Attachment direction bhi senderType ke hisaab se adjust */
                                                                    <div className={`mt-2 p-2 border rounded d-flex align-items-center ${msg.senderType === 'admin' ? 'justify-content-end' : ''}`} style={{ width: 'fit-content', marginLeft: msg.senderType === 'admin' ? 'auto' : '0', marginRight: msg.senderType === 'user' ? 'auto' : '0' }}>
                                                            ...
                                                            {/* AVATAR RIGHT (Admin ka logo right side aayega) */}
                                                            {msg.senderType === 'admin' && (
                                                                <div className="avatar ms-2" style={{ minWidth: '35px', height: '35px', borderRadius: '50%', overflow: 'hidden', background: '#007bff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
